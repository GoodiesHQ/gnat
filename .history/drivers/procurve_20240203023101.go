...
func (procurve *NetworkSwitchProcurve) Cmd(ctx context.Context, command string) (*SwitchResult, error) {
	...
	procurve.Conn.Send(buf.Bytes())

	// read until the desired regex
	data, err := procurve.Conn.ReadUntilMatch(ctx, procurve.RegexCmd(), procurve.ReadTimeout)
	if err != nil {
		return nil, err
	}
