...
func (procurve *ProcurveDevice) Initialize(ctx context.Context) error {
	...
	data, err := procurve.Connection.ReadUntilMatch(ctx, procurve.TimeoutRead, sequences)
	if err != nil {
		log.Error().Err(err).Msg("Failed to read untul match in Initialize()")
		return err
	}
	log.Info().Bytes("data", data).Msg("Data found")
